...
def dateIndex(listName):
    ...
    #print("7,", datesOnly)
    MarTwenty=datesOnly.index('03/21 02:16')
    JanFirst=datesOnly.index('01/01 02:16')
    OctFirst=datesOnly.index('10/16 02:16')
...
def main(fileName):
    ...
    for i in range(len(timesYN)):
        ...
        if timesYN[i][0].endswith("02:16")==True:
            NineList.append(timesYN[i][1])
        if timesYN[i][0].endswith("03/21/ 02:16"):
            NineList.append("This is the beginning of the snow season")
        if timesYN[i][0].endswith("06/30/ 20:16"):
